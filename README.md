...
Stages are `fast`, `normal`, or `slow`. Their default per-answer limits are 5, 10, and 15 seconds. A correct answer receives points based on the problem's complexity; in soft mode, it receives full points through the limit, then a linear reduction to zero at twice the limit. Input remains available after that point. `--strict` turns the limit into an immediate failed attempt. `--cooldown` gives a short transition pause before the next example.
